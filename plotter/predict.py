...
def combined_hist2d(true, pred, bins=20, savename=""):
    """
    structure in data is [N, 2], where N is number of samples and 2 is TOA 1. SW and 2. LW
    true: True values
    pred: predicted values
    
    """
    def marginals_2dhist(x, y, ax, ax_histx, ax_histy, nbins=20, title=""):
        
        fn=16
        # no labels
        ax_histx.tick_params(axis="both", labelbottom=False, left=False, labelleft=False)
        ax_histy.tick_params(axis="both", labelleft=False, labelbottom=False, bottom=False)
        min1 = np.min(x)
        max1 = np.max(x)
        ax.plot([min1,max1],[min1,max1], color="r")
        im = ax.hist2d(x, y, range=[[min1,max1],[min1,max1]], bins=nbins, density=True, norm=cl.LogNorm())
        ax.tick_params(axis='both', which='major', labelsize=fn)
        ax.set_ylabel('Predicted',fontsize = fn)
        ax.set_xlabel('True',fontsize = fn)
        ax_histx.set_title(title,fontsize = fn)
        ax.set_xlim(min1,max1)
        ax.set_ylim(min1,max1)
        binwidth = (max1-min1)/nbins
        hist_bins = np.arange(min1, max1+binwidth, binwidth)
        ax_histx.hist(x, bins=hist_bins, alpha=0.5)
        ax_histx.set_yscale("log")
        ax_histy.hist(y, bins=hist_bins, orientation='horizontal', alpha=0.5)
    ...
    gs = fig.add_gridspec(2, 7,  width_ratios=(5, 1, 0.2, 1.5, 5, 1, 0.2), height_ratios=(1, 5), wspace=0.0, hspace=0.0) #, left=0.1, right=0.9, bottom=0.1, top=0.9, )
    # Create the Axes.
    ax_kwargs = {"frame_on": False}
    lax = fig.add_subplot(gs[1, 0])
    lax_histx = fig.add_subplot(gs[0, 0], sharex=lax, **ax_kwargs)
    lax_histy = fig.add_subplot(gs[1, 1], sharey=lax, **ax_kwargs)
    lcax = fig.add_subplot(gs[1, 2])
    rax = fig.add_subplot(gs[1, 4], ymargin=10)
    rax_histx = fig.add_subplot(gs[0, 4], sharex=rax, **ax_kwargs)
    rax_histy = fig.add_subplot(gs[1, 5], sharey=rax, **ax_kwargs)
    rcax = fig.add_subplot(gs[1, 6])
    # Draw the scatter plot and marginals.
    lim = marginals_2dhist(true[:,0],  pred[:,0], lax, lax_histx, lax_histy, nbins=bins, title='Outgoing Shortwave Radiation [$W/m^2$]')
    rim = marginals_2dhist(true[:,1],  pred[:,1], rax, rax_histx, rax_histy, nbins=bins, title='Outgoing Longwave Radiation [$W/m^2$]')
    cbar_l = fig.colorbar(lim[3], cax=lcax)
    cbar_l.set_label("Probability")
    cbar_r = fig.colorbar(rim[3], cax=rcax)
    cbar_r.set_label("Probability")
...
def plot_latent(org, bins = 20, cmap = "viridis", diag_norm="log", norm = cl.LogNorm(), savename=""):
    """
    org with shape [n, n_org]
    """
    norm = norm
    bins = bins

    n = len(org[0])
    fig, ax = plt.subplots(n,n, sharex="col", figsize=(int(n*2+1),int(n*2)))
    for i in range(n):
        for j in range(n):
            idx = int(n*i+j)
            #ax = plt.subplot(n,n,int(n*i+j+1))
            if i ==j:
                ax[i,j].hist(org[:,i], bins=bins, density=True) #  label=
                ax[i,j].set_ylabel("Probability")
                ax[i,j].set_xlabel(f"org {i+1}")
                ax[i,j].set_yscale(diag_norm)
                #ax[i,j].legend()
            else:
                im = ax[i,j].hist2d(org[:,j], org[:,i], bins=bins, cmap=cmap, norm=norm) # density=True,
                ax[i,j].set_xlabel(f"org {j+1}")
                ax[i,j].set_ylabel(f"org {i+1}")
                #plt.colorbar(im[3] , ax=ax[i,j])            
...
def two_hist(x, y, y_label="", bin_range=[None, None], bins=20, cmap = "viridis", norm = cl.LogNorm(), savename=""):
    title = ["OSR", "OLR"]
    plt.figure()
    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(10,5))
    for i in range(2):
        ax = axes[i]
        min_x, max_x = min(x[i]), max(x[i])
        im = ax.hist2d(x[i], y[i], bins = bins, cmap=cmap, range=bin_range[i], density=True, norm=norm)
        ax.set_xlabel(f"True {title[i]} [$W/m^2$]")
        text = "$\\bar{x}$ = " + str(np.round(np.mean(y[i]), 2)) + "; $\\bar{|x|}$ = " + str(np.round(np.mean(np.abs(y[i])), 2)) + "; median: " + str(np.round(np.median(y[i]), 2))
        ax.set_title(text)
        ax.set_ylabel(y_label)
        ax.plot([min_x, max_x], [0, 0], color="red")
